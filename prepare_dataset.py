    ...
    for subset in tqdm(os.listdir(train_dir), desc="subset"):
        ...
        if len(subset_images) > 0:
            for image_filepath in tqdm(subset_images, desc="image"):
                ...
                image_output_dir = os.path.join(args.output_path, "train", subset, "images")
                os.makedirs(image_output_dir, exist_ok=True)
                cv2.imwrite(os.path.join(image_output_dir, image_filepath[:-3] + "png"), image)
        if len(subset_labels) > 0:
            for label_filepath in tqdm(subset_labels, desc="label"):
                label = cv2.imread(os.path.join(train_dir, subset, "labels", label_filepath), cv2.IMREAD_GRAYSCALE)
                ...
                np.save(os.path.join(label_npy_output_dir, label_filepath[:-3] + "npy"), label_npy)
