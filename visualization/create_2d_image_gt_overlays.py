...
def do_parsing():
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Visualize images with gt overlay"
    )
...
def main():
    ...
    for subset in tqdm(os.listdir(train_dir), desc="subset"):
        ...
        try:
            assert len(subset_images) == len(
                subset_labels
            ), f"Number of images {len(subset_images)} != number of labels {len(subset_labels)} for subset {subset}"
            print(
            ...
            if len(subset_images) > 0:
                ...
                for image_filepath in tqdm(subset_images, desc="image"):
                    ...
                    label = cv2.imread(
                        os.path.join(train_dir, subset, "labels", image_filepath),
                        cv2.IMREAD_GRAYSCALE,
                    )
                    # Convert label to red
                    label = np.expand_dims(label, axis=-1)
                    label_red = np.concatenate(
                        [np.zeros_like(label), np.zeros_like(label), label], axis=-1
                    )
                    ...
                    if args.debug:
                        ...
                        cv2.waitKey(0)
                    cv2.imwrite(
                        os.path.join(output_dir, image_filepath[:-3] + "png"),
                        image_pairs,
                    )
